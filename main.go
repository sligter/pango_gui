						...
						return layout.Flex{Axis: layout.Vertical}.Layout(gtx,
							...
							layout.Rigid(func(gtx layout.Context) layout.Dimensions {
								return layout.Inset{Top: unit.Dp(8)}.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
									if state.FolderPickerBtn.Clicked(gtx) {
										go func() {
											folderPath, err := dialog.Directory().Title("Select Pango Directory").Browse()
											if err == nil {
												state.UploadDir.SetText(folderPath)
											}
										}()
									}
									return material.Button(th, &state.FolderPickerBtn, "Select Pango Directory").Layout(gtx)
								})
							}),
						)
					}),
